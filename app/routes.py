...
from app.speech import (
    speech_to_speech, speech_to_text, text_to_speech,
    save_audio_file, validate_session_continuity, update_session_voice_usage
)
...
def validate_session():
    try:
        ...
        if not session_id:
            return jsonify({"valid": False, "message": "No session ID provided", "status": "error"}), 400
        ...
        return jsonify({"valid": False, "message": f"Validation error: {str(e)}", "status": "error"}), 500
...
def end_session():
    try:
        ...
        return jsonify({"status": "error", "message": f"Error ending session: {str(e)}"}), 500
...
def handle_speech_to_speech():
    try:
        ...
        if not session_id or session_id in ['null', 'undefined', '']:
            return jsonify({"error": "Session ID is required", "message": "Please create a session first using /session/create", "status": "error"}), 400

        session_validation = validate_session_continuity(session_id)
        if not session_validation["valid"]:
            return jsonify({"error": "Invalid session ID", "message": session_validation["message"], "status": "error"}), 400
        ...
        if not result or "error" in result:
            ...
            return jsonify({"error": error_msg, "message": "No speech detected or processing failed. Please try again.", "session_id": session_id, "status": "error"}), 400
...
def language_change():
    try:
        ...
        if not validation["valid"]:
            return jsonify({"status": "error", "message": f"Invalid session: {validation['message']}"}), 400

        # Update map (behavior from version 1)
        session_language_map[session_id] = to_language

        # Store a system message (behavior from version 2)
        message = f"[SYSTEM] Language changed from {from_language} to {to_language}. All responses should now be in {'Dutch' if to_language == 'nl-NL' else 'English'}."
        success = store_message(session_id, message, "system")

        if success:
            return jsonify({"status": "success", "message": f"Language changed to {to_language}"}), 200
        else:
            return jsonify({"status": "error", "message": "Failed to record language change"}), 500
...
def check_consent(session_id):
    return jsonify(check_consent_status(session_id))
